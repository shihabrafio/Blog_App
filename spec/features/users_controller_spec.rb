...
describe 'user/index', type: :feature do
  before(:all) do
    @user1 = User.create(name: 'Alberto', photo: 'photo_url', bio: 'Bike Rider')
    @user2 = User.create(name: 'Yara', photo: 'photo_url_2', bio: 'Plants Lover')
  end
  ...
  it 'should redirect to that user\'s show page on clicking a user' do
    @user3 = User.create(name: 'Ana', photo: 'photo_url_2', bio: 'Musician')
...
describe 'user/show', type: :feature do
  before(:all) do
    @user1 = User.create(name: 'Tim', photo: 'photo_url', bio: 'Bike Rider')
  ...
  it 'shows the profile picture, number of posts and bio for each user' do
    ...
    expect(page).to have_content('Bike Rider')
  end
